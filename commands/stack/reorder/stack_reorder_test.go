...
func Test_matchBranchesToStack(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = matchBranchesToStack(tt.args.stack, tt.args.branches)

			if tt.wantErr {
				require.Error(t, err)
			} else {
				matchedStack, err := git.GatherStackRefs("cool stack")
				require.Nil(t, err)

				for k, ref := range tt.expected.Refs {
					require.Equal(t, ref, matchedStack.Refs[k])

					wantpath := path.Join(dir, git.StackLocation, ref.SHA+".json")
					require.True(t, config.CheckFileExists(wantpath))

				}

				require.Equal(t, len(tt.args.branches), len(matchedStack.Refs))
			}
		})
	}
}
