...
func NewCmdList(f *cmdutils.Factory) *cobra.Command {
	...
	pipelineListCmd.Flags().StringP("ref", "r", "", "Only return pipelines for given ref")
	pipelineListCmd.Flags().String("scope", "", "Only return pipelines with the given scope: {running|pending|finished|branches|tags}")
	pipelineListCmd.Flags().String("source", "", "Only return pipelines triggered via the given source: {pi|chat|external|external_pull_request_event|merge_request_event|ondemand_dast_scan|ondemand_dast_validation|parent_pipeline|pipeline|push|schedule|security_orchestration_policy|trigger|web|or webide}")
	pipelineListCmd.Flags().String("sha", "", "Only return pipelines with the given sha.")
	pipelineListCmd.Flags().BoolP("yaml-errors", "y", false, "Only return pipelines with invalid configurations")
	pipelineListCmd.Flags().StringP("name", "n", "", "Only return pipelines with the given name")
	pipelineListCmd.Flags().StringP("username", "u", "", "Only return pipelines triggered by the given username")
	pipelineListCmd.Flags().StringP("updated-before", "b", "", "Only return pipelines updated before the specified date. Expected in ISO 8601 format (2019-03-15T08:00:00Z).")
	pipelineListCmd.Flags().StringP("updated-after", "a", "", "Only return pipelines updated after the specified date. Expected in ISO 8601 format (2019-03-15T08:00:00Z).")
