...
```plaintext
  ...
  -r, --ref string              Return only pipelines for given ref.
      ...
      --source string           Return only pipelines triggered via the given source: {pi|chat|external|external_pull_request_event|merge_request_event|ondemand_dast_scan|ondemand_dast_validation|parent_pipeline|pipeline|push|schedule|security_orchestration_policy|trigger|web|webide}
