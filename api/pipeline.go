...
var CreateMRPipeline = func(client *gitlab.Client, projectID interface{}, mrID int) (*gitlab.PipelineInfo, error) {
	if client == nil {
		client = apiClient.Lab()
	}

	pipe, _, err := client.MergeRequests.CreateMergeRequestPipeline(projectID, mrID)
	if err != nil {
		return nil, err
	}
	return pipe, nil
