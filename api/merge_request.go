...
var UpdateMRNotes = func(client *gitlab.Client, projectID interface{}, mrID int, noteID int, opts *gitlab.UpdateMergeRequestNoteOptions) (*gitlab.Note, error) {
	if client == nil {
		client = apiClient.Lab()
	}

	note, _, err := client.Notes.UpdateMergeRequestNote(projectID, mrID, noteID, opts)
	if err != nil {
		return nil, err
	}
